"""Insight-table brief builders (Phase 3) -- recent consolidation candidates
and operator-reviewed lessons.

Extracted from cognition/brief_sections_organ.py during the SLOC
decomposition. Both builders read the ``insights`` table, low-signal-filter
the rows, render a capped section, and best-effort stamp ``last_surfaced_at``
/ ``surface_count`` on the rows they surface. Failure-soft for pre-migration
DBs.
"""
