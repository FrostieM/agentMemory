...
Claude Code installation:
  ...
  1. Project-scoped ``.claude/settings.json`` gains a
     ``UserPromptSubmit`` hook → ``inject_memory_brief_v3.py`` and a
...
Usage::
    ...
    python scripts/install_v3_hooks.py --project C:\\path\\to\\repo

    # Apply for real:
    python scripts/install_v3_hooks.py --project C:\\path\\to\\repo --apply

    # Hooks only, skip seed step:
    python scripts/install_v3_hooks.py --project C:\\path\\to\\repo --apply --no-seed

    # Seed only, skip hook wiring:
    python scripts/install_v3_hooks.py --project C:\\path\\to\\repo --apply --no-hooks
"""
...
BRIEF_HOOK_SCRIPT = REPO_ROOT / "scripts" / "inject_memory_brief_v3.py"
POSTEDIT_HOOK_SCRIPT = REPO_ROOT / "scripts" / "post_edit_enqueue.py"
SEED_SCRIPT = REPO_ROOT / "scripts" / "seed_v3_discipline.py"
...
def build_plan(
    *,
            ...
            HookChange(
                ...
                status=(
                    ...
                    if _hook_already_installed(
                        current, "UserPromptSubmit", "inject_memory_brief_v3"
                    )
                    else "pending"
                ),
            )
    ...
    if install_seed:
        # Names match scripts/seed_v3_discipline.py — exposed for the report
