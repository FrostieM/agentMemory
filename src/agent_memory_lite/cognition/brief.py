...
def _build_pinned_behaviors(
    ...
    implementation pulled top-12 pinned behaviors via list_kind which
    sorts by ``updated_at DESC``. With 20+ pinned, recently-updated
    ``project_convention`` rules pushed safety-critical
    ``user_preference`` rules out of the brief. The agent then made
    7 unauthorized pushes to main because the pinned ``no-git-push-
    without-explicit-permission`` rule was in the DB but not in its
    visible context.

    Also: behaviors live in TWO tables — the legacy ``behaviors``
    table (written by compaction.promote_insight_to_behavior) and
    the canonical ``behavior_instructions`` table (written by
    /memory/upsert_behavior_instruction). The brief used to read
    only ``behaviors``. Now we read both and dedupe by name.

    Priority weights mirror ``behavior_repo_ranking.PRIORITY_WEIGHT``:
    system_bound > user_preference > project_convention > suggestion.
    """
    candidates = _collect_pinned_behaviors_both_tables(conn, workspace_id)
    # Sort by priority weight DESC, then updated_at DESC (recency
    # breaks priority ties so a freshly-restated rule wins over a
    # stale one of the same priority).
    candidates.sort(
        key=lambda b: (
...
def _collect_pinned_behaviors_both_tables(
    ...
    The brief composer used to read only ``behaviors`` via
    ``list_kind``. Operator behaviors written via HTTP
    /memory/upsert_behavior_instruction land in
    ``behavior_instructions``; they were invisible to the agent's
    brief — the dominant cause of the 2026-05-20 push-without-
    approval incident.
    """
