...
Stable repo guidance for agents working on this project.

## Project Shape

`agent-memory-lite` is a local-first memory service for AI coding agents.
SQLite is the source of record, LanceDB stores vectors, sentence-transformers
does local embeddings, and Ollama is the optional local LLM extractor. The
service binds to `127.0.0.1:8765`.

## Hard Rules

- Active agent surface is v3-only.
- Do not add new legacy/v2 MCP names or compatibility shims.
- Keep legacy references only in historical changelogs and migrations.
- Use forward-only SQL migrations.
- Keep source modules small and focused.
- Use paired tests for non-trivial behavior.
- Keep code, comments, commits, and docs in English.
- Never store secrets.
...
maintenance/     background loops and quality checks
...
python scripts/memory_audit.py --workspace agent-memory-lite --json
```

The canonical contract below is synced from `docs/AGENT_CONTRACT.md`.
