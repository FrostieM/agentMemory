...
> "По просьбе они могут прочитать другую память, только явно. Просто
> так читать и записывать в чужую память он не должен вообще."
>
> "On request they can read other memory, but only explicitly. Just
> reading or writing to a foreign memory should not happen at all."
...
"посмотри память copyBot" was a legitimate explicit request that the
