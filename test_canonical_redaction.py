"""Test that write_canonical DOES redact secrets properly."""
import sqlite3
import json
...
tmp_db = Path("/tmp/test_canonical.db")
if tmp_db.exists():
    tmp_db.unlink()
...
# Write an insight with secrets via write_canonical (proper API)
...
if row:
    row_dict = dict(row)
    print(f"Insight row summary via write_canonical: {row_dict.get('summary')}")
    ...
    if secrets_found:
        print("\n[ISSUE] Secrets found in cleartext when using write_canonical:")
        for secret in secrets_found:
            print(f"  - {secret}")
    else:
        print("\n[OK] Secrets are properly redacted with write_canonical")
else:
    print("No row found")
