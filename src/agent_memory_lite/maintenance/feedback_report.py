...
def _summaries(
    ...
    *,
    ...
    summaries: list[FeedbackSourceSummary] = []
    for row in rows:
...
def run_usage_feedback_report(
    ...
    *,
    ...
    if not _table_exists(conn, "memory_usage_feedback"):
    ...
    summaries = _summaries(conn, workspace_id=workspace_id, limit=limit, query_limit=3)
    noisy_sources = [
        item
