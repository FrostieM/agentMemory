"""Unit tests for scripts/install_v3_hooks.py.
...
import pytest
from scripts import install_v3_hooks as installer
            ...
            "UserPromptSubmit": [
                {
                    "hooks": [
                        {
                            "type": "command",
                            "command": '"python" "...inject_memory_brief_v3.py"',
                        }
                    ]
                }
            ]
...
def test_apply_hooks_writes_expected_structure(project: Path, registry_file: Path) -> None:
    ...
    assert "inject_memory_brief_v3" in user_prompt_entries[0]["hooks"][0]["command"]
