...
def memory_write_resolve_candidates_instruction(
    ...
    """Force candidate resolution + capability-context capture in the same turn."""
    return BehaviorInstructionIn(
        workspace_id=workspace_id,
        name="memory-write-is-not-done-until-candidates-resolved",
        rule=(
            "TRIGGER: memory_write or memory_edit response has candidates_written > 0, "
            ...
            "  3. IF capability_suggestions top-1 has strength >= 0.6: record "
            "that top-1 suggestion in the current task, plan_step, or final episode.\n"
            "  4. IF source_episode_id == null: either allow_orphan=true with a one-line "
            "reason, OR memory_write(kind=episode) first and re-issue the write.\n\n"
        ...
        applies_to=[
            ...
            "memory_write kind=episode response",
            "memory_write kind=decision response",
            "memory_write kind=theory response",
